...
import { CellDiet, IEnergySource, IEnergySourceConstructor } from './CellDiet';
import { CellPhysic } from './CellPhysic';
import { CellRelations } from './CellRelations';
import { CellRenderer } from './CellRenderer';
import { CellSenses } from './CellSenses';
import { CellState, IStateScreenshot } from './CellState';

let lastId = 0;

export class Cell {
  private readonly behavior = new CellBehavior(this);
  private readonly diet = new CellDiet(this);
  private readonly relations = new CellRelations(this);
  private readonly renderer = new CellRenderer(this);
  private readonly senses = new CellSenses(this);
  private readonly state = new CellState(this);
  private readonly body = new CellBody(this, this.state);
  private readonly physic = new CellPhysic(this, this.state);
  private readonly emitter = new Emitter();

  constructor(readonly id = lastId++) {
    this.flushState();
  }
