...
export function fromAngle(angle: number, mag = 1): Vector {
...
export function dotProduct(left: Vector, right: Vector) {
  return left.x * right.x + left.y * right.y;
}

export function invertVector(point: Vector) {
  return vectorAxis((axis) => -point[axis]);
}
